...
Det här dokumentet kompletterar [deployment.md](deployment.md). Deploymentdokumentet beskriver appens versionerade provisioning-/migreringsordning; här ligger verifierings- och incidentmodellen.
...
- Worker-validering genom appens `validate:worker`-script.
...
Appens versionerade deploykommando är:
...
Vilket externt CI/CD-system som eventuellt anropar kommandot är inte app-local current-state och dokumenteras inte här.
...
- GitHub numeric ID används enligt appens allowlistmodell,
...
- Dynamic Client Registration och Client Credentials ska förbli avstängda om inte appens arkitektur uttryckligen ändras.
...
`apps/krosa-maja/wrangler.jsonc` är appens auktoritativa deploybara runtimekonfiguration för:
...
Adminfunktioner ska fortsatt kräva den authorization som repositoryts server-side implementation upprätthåller. Externa edge-/Access-policyer är separat live-state och dokumenteras inte som app-current-state här.
