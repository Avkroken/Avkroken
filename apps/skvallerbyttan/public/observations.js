...
function withRefresh(path, force) {
...
async function loadGitHub(force = false) {
  ...
  try {
    const data = await api(withRefresh("/api/v1/github/org/state", force), "github-org-state");
...
async function loadCloudflare(force = false) {
  ...
  try {
    const [account, zones, workers, d1, kvStorage, r2, access, tunnels, audit] = await Promise.all([
      ...
      api(withRefresh("/api/v1/cloudflare/storage/d1", force), "cf-storage-d1"),
...
function renderProviderHealth(health) {
  ...
  $("#provider-health").innerHTML = ["github", "cloudflare"].map((provider) => {
    ...
    const installation = item.auth?.installation || {};
    const permissionSource = Object.keys(installation.tokenPermissions || {}).length
      ? installation.tokenPermissions
      : installation.permissions || {};
    const permissionText = Object.entries(permissionSource)
    ...
    return `<article class="health-row">
      ...
      <div class="health-details">
        ...
        ${provider === "github" ? `<span>Repository selection: ${esc(installation.repositorySelection || "—")}</span>` : ""}
        ${provider === "github" ? `<span>App permissions: ${esc(permissionText || "not_observed")}</span>` : ""}
  ...
  $("#capability-cards").innerHTML = (caps.capabilities || []).map((cap) => `
    <article class="capability-card">
      ...
      ${kv([
        ...
        ...(cap.scopeCoverage ? [[
          "Scope coverage",
          `${fmtInt(cap.scopeCoverage.observed)}/${fmtInt(cap.scopeCoverage.expected)} observed · ${fmtInt(cap.scopeCoverage.available)} available · ${fmtInt(cap.scopeCoverage.permissionDenied)} denied · ${fmtInt(cap.scopeCoverage.error)} error`,
        ]] : []),
      ...
      <details>
        ...
        ${cap.acceptedPermissions ? `<p class="small">GitHub accepted permissions: ${esc(cap.acceptedPermissions)}</p>` : ""}
...
async function loadInsight(force = false) {
  ...
  $("#insight-status").textContent = force
    ? "Uppdaterar provider-state och capability-registret…"
    : "Laddar capability-registret…";
  try {
    if (force) {
      await api(
        withRefresh("/api/v1/capabilities", true),
        "insight-capabilities-refresh",
      );
    }
...
export async function refreshAllObservationData(force = true) {
  if (!force) {
    await ensureTabData(view.activeTab);
    return;
  }

  // "Uppdatera" is a dashboard-wide operator action. Reconcile provider and
  // capability state first, then re-render every observation view from the
  // newly refreshed canonical caches regardless of which tab is active.
  await loadInsight(true);

  view.loaded.delete("github");
  view.loaded.delete("cloudflare");
  view.loaded.delete("activity");
