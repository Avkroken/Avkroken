...
import {
  isGitHubAppWebhookPayload,
  isRetiredGitHubAppWebhook,
  portalDocsInvalidation,
...
test("distinguishes retired GitHub App webhook payloads from organization webhook payloads", () => {
  const appPayload = JSON.stringify({
    installation: { id: 123 },
    organization: { login: "Avkroken" },
  });
  const organizationPayload = JSON.stringify({
    organization: { login: "Avkroken" },
    repository: { name: "Skvallerbyttan" },
  });

  assert.equal(isGitHubAppWebhookPayload(appPayload), true);
  assert.equal(isGitHubAppWebhookPayload(organizationPayload), false);
  assert.equal(isGitHubAppWebhookPayload("{not-json"), false);

  assert.equal(isRetiredGitHubAppWebhook("integration", organizationPayload), true);
  assert.equal(isRetiredGitHubAppWebhook("organization", appPayload), true);
  assert.equal(isRetiredGitHubAppWebhook("organization", organizationPayload), false);
});
...
test("custom property webhooks invalidate governance and effective repository state", () => {
  assert.deepEqual(webhookCacheKeys("custom_property", null), [
    "overview",
    "github:org:governance",
  ]);
  assert.deepEqual(webhookCacheKeys("custom_property_values", "Skvallerbyttan"), [
    "overview",
    "repository:Skvallerbyttan",
    "insights:Skvallerbyttan",
    "github:org:governance",
    "github:repo:Skvallerbyttan:effective-policy",
  ]);
});
