...
test("GitHub webhook activity classifies custom-property and issue-relation events", () => {
  const property = activityFromGitHubWebhook(
    "delivery-property",
  ...
  const relation = activityFromGitHubWebhook(
    "delivery-relation",
    "issue_dependencies",
    "Skvallerbyttan",
    { action: "blocked_by_added", issue: { number: 17 } },
    "2026-09-22T05:00:01.000Z",
  );

  assert.equal(property.capability, "github.avkroken.custom_properties");
  assert.equal(relation.capability, "github.avkroken.pull_requests");
});
