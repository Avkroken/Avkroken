...
export function authConfigured(env: Env): boolean {
  return Boolean(
    ...
    secretValueConfigured(env.GITHUB_OAUTH_CLIENT_SECRET) &&
...
async function exchangeCode(env: Env, code: string, verifier: string): Promise<string> {
  const clientSecret = await resolveSecretValue(env.GITHUB_OAUTH_CLIENT_SECRET);
...
async function revokeGitHubToken(env: Env, accessToken: string): Promise<void> {
  ...
  const credentials = btoa(`${env.GITHUB_OAUTH_CLIENT_ID}:${clientSecret}`);
  try {
    await fetch(
...
<body>
  <main class="login-shell">
    ...
    <section class="login-card" aria-labelledby="login-title">
      ...
      <p>GitHub verifierar din identitet. Endast uttryckligen tillåtna GitHub-konton släpps in.</p>
      ${alert}
      ${button}
