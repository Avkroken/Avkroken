...
function githubCapability(event: string): string {
  ...
  if (event === "custom_property" || event === "custom_property_values") {
    return "github.avkroken.custom_properties";
  }
...
export async function getObservedActivity(
  ...
  if (!env.STATS_DB) return { schemaVersion: 2, available: false, status: "not_configured", reason: "d1-not-bound" };
