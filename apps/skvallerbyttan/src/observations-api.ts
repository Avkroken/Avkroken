...
import {
  ...
  loadGitHubRepositoryEffectivePolicy,
...
const MINUTE = 60_000;
const OVERVIEW_TTL_MS = 6 * 60 * MINUTE;
...
function json(value: unknown, status = 200, headers?: HeadersInit): Response {
  ...
  output.set("X-Skvallerbyttan-Schema-Version", "2");
...
async function cached<T>(
  ...
  if (force) {
    ...
    return json(value, 200, {
      ...
      "X-Skvallerbyttan-Refresh": "forced",
    });
  }
...
async function overviewWithHistory(
  env: Env,
  context: ExecutionContext,
...
export async function refreshOverviewSource(
  env: Env,
  context: ExecutionContext,
): Promise<Record<string, unknown>> {
  return refreshSingleFlight(
    ...
    "overview",
    {
      capability: "github.avkroken.repositories",
      provider: "github",
...
export async function reconcileGitHubCapabilitySources(env: Env): Promise<void> {
  const overview = await getOverview(env);
  const repositories = Array.isArray((overview as { repositories?: unknown }).repositories)
    ? (overview as { repositories: Array<Record<string, unknown>> }).repositories
    : [];
  const repoNames = repositories
    .map((item) => typeof item.name === "string" ? item.name : null)
    .filter((item): item is string => Boolean(item));
  const scopeIds = repositories
    .map((item) => typeof item.fullName === "string" ? item.fullName : null)
    .filter((item): item is string => Boolean(item));

  const rulesetObservations = await mapLimit(
    repoNames,
    3,
    async (repoName, index): Promise<CapabilityScopeObservationInput> => {
      let observation: CapabilityScopeObservationInput | null = null;
      await refreshSingleFlight(
        env,
        `github:repo:${repoName}:effective-policy`,
        "governance",
        {
        ...
        async () => {
          ...
          observation = loaded.rulesetObservation;
          return loaded.value;
        },
        "reconciliation",
      );
      if (!observation) {
        return {
          scopeId: scopeIds[index] ?? repoName,
          status: "error",
          permissionState: "unknown",
          dataState: "error",
          httpStatus: 0,
          error: "github-ruleset-reconciliation-missing-observation",
        };
      }
      return observation;
    },
  ...
  await recordCapabilityScopeBatch(
    env,
...
async function reconcileCloudflareNotifications(env: Env): Promise<unknown> {
  return cloudflareRead(env, "cloudflare.avkroken.notifications", async () => {
    const [history, policies, webhooks] = await Promise.all([
      refreshSingleFlight(
        env,
        "cloudflare:notifications:history",
        "cloudflare",
        {
          capability: "cloudflare.avkroken.notifications",
          provider: "cloudflare",
          consumer: "reconciliation",
        },
    ...
    return { schemaVersion: 2, available: true, history, policies, webhooks };
  });
}

async function reconcileCloudflareZeroTrust(env: Env): Promise<unknown> {
  return cloudflareRead(env, "cloudflare.avkroken.zero_trust", async () => {
    const casbWebhooks = await refreshSingleFlight(
      env,
      "cloudflare:casb:webhooks",
      "cloudflare",
      {
        capability: "cloudflare.avkroken.zero_trust",
        provider: "cloudflare",
        consumer: "reconciliation",
      },
    ...
    return { schemaVersion: 2, available: true, casbWebhooks };
  });
}
...
export async function reconcileObservationSources(env: Env): Promise<void> {
  const tasks: Array<Promise<unknown>> = [
    ...
    reconcileCloudflareNotifications(env),
    reconcileCloudflareZeroTrust(env),
  ];
...
export async function reconcileAllCapabilitySources(env: Env): Promise<void> {
  await Promise.all([
    reconcileGitHubCapabilitySources(env),
    reconcileObservationSources(env),
  ]);
}
...
export async function handleObservationApi(
  ...
  const forceRefresh = consumer === "dashboard" && url.searchParams.get("refresh") === "1";

  if (request.method !== "GET") {
    return json({ schemaVersion: 2, error: "method_not_allowed" }, 405, { Allow: "GET" });
  }
  ...
  if (url.pathname === "/api/v1/security-activity") {
    const rawRepo = url.searchParams.get("repo");
    const repo = rawRepo === null ? null : validRepo(rawRepo);
    if (rawRepo !== null && repo === null) {
      return json({ schemaVersion: 2, error: "invalid_repository_name" }, 400);
    }
    const days = Number(url.searchParams.get("days") ?? "30");
    const startedAt = Date.now();
    try {
      const value = await getSecurityActivity(
        env,
        repo,
  ...
  const repositoryInsightMatch = url.pathname.match(/^\/api\/v1\/repos\/([^/]+)\/insights$/);
  if (repositoryInsightMatch) {
    const repo = validRepo(repositoryInsightMatch[1]);
    if (!repo) return json({ schemaVersion: 2, error: "invalid_repository_name" }, 400);
    return cached(
      env,
      context,
      `insights:${repo}`,
  ...
  const repositoryMatch = url.pathname.match(/^\/api\/v1\/repos\/([^/]+)$/);
  if (repositoryMatch) {
    const repo = validRepo(repositoryMatch[1]);
    if (!repo) return json({ schemaVersion: 2, error: "invalid_repository_name" }, 400);
    return cached(
      env,
      context,
  ...
  if (url.pathname === "/api/v1/capabilities") {
    if (forceRefresh) await reconcileAllCapabilitySources(env);
    const value = await getCapabilities(env);
    return json({ ...value, providerHealth: getProviderHealth(env, value.capabilities) });
  }

  if (url.pathname === "/api/v1/provider-health") {
    const value = await getCapabilities(env);
    return json(getProviderHealth(env, value.capabilities));
  }
  ...
  return json({ schemaVersion: 2, error: "not_found" }, 404);
}
