...
export function isGitHubAppWebhookPayload(body: string): boolean {
  try {
    const parsed = JSON.parse(body) as { installation?: unknown } | null;
    return Boolean(
      parsed
      && typeof parsed === "object"
      && parsed.installation
      && typeof parsed.installation === "object",
    );
  } catch {
    return false;
  }
...
export function isRetiredGitHubAppWebhook(
  hookTargetType: string,
  body: string,
): boolean {
  return hookTargetType.toLowerCase() === "integration" || isGitHubAppWebhookPayload(body);
}
...
export async function handleGitHubWebhook(request: Request, env: Env): Promise<Response> {
  ...
  const hookId = request.headers.get("x-github-hook-id")?.trim() || "";
  ...
  if (isRetiredGitHubAppWebhook(hookTargetType, body)) {
    return response({ ok: true, ignored: "retired github app webhook" }, 202);
  }
